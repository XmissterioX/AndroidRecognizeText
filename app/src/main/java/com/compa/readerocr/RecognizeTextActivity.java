...
/**
 * Put the text cleanup method here ! We need to keep numbers only!!!
 * @author Sea
 *
 */
public class RecognizeTextActivity extends Activity {
	...
	/**
	 * Here we take the picture and analyze its content
	 * @param requestCode
	 * @param resultCode
	 * @param data
	 */
	...
	public void displayResult(Bitmap imageBitmap, int top, int bot, int right, int left) {
		...
		if (processImg.parseBitmap(imageBitmap, top, bot, right, left)) {
			...
			recognizeResult.setText(processImg.recognizeResult.replaceAll("[^0-9]", ""));
