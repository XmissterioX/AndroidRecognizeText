...
public class CameraPreview implements SurfaceHolder.Callback {
	...
	PictureCallback picCallback_JPG = new PictureCallback() {
		...
		public void onPictureTaken(byte[] arg0, Camera arg1) {
			...
			info(String.valueOf(TOP));
			info(String.valueOf(BOT));
			info(String.valueOf(RIGHT));
			info(String.valueOf(LEFT));
	...
	private List<Rect> createAreaSign(int width, int height) {
		...
		// unknown value used to calculate the box size
		...
		// Calculating box margins on the sides
		marginLeftRight = avrg / 2;
		info("Avrg: " + avrg + "  boxH=boxW: " + boxH + "  marginLR: " + marginLeftRight);

		// Starting point
		int beginTop = startX;
		// Setting the top left point y position (along the height)
...
//		LEFT = (int) ((topLeftX + halfBox) * ratioImg);
		...
		// TL
		int pLeft = width/4;
		int pTop = (int)marginLeftRight*10;
		int pRight = (int)(pLeft * 1.2f);
		int pBot = height-(int)marginLeftRight*10;
		...
		protected void onDraw(Canvas canvas) {
			if (lstRectArea != null && lstRectArea.size() > 0) {
				for (Rect rect : lstRectArea) {
					canvas.drawRect(rect.left * 1F, rect.top * 1.5F , rect.right * 1.0F, rect.bottom * 1.0F,
							drawingPaint);
				}
			}
		}
